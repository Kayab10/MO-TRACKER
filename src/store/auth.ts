...
import { cloudEnabled, pull, push } from '../lib/cloud';
...
/** Users come from the cloud when configured, else localStorage; seeded with defaults on first run. */
async function loadUsers(): Promise<User[]> {
  ...
  if (!users.length) {
    ...
    db.setUsers(users); // refresh local cache
  }
  return users;
}
...
async function saveUsers(users: User[]) {
  db.setUsers(users);
  if (cloudEnabled) await push('users', users);
}
...
export async function changePassword(
  ...
  if (u.passwordHash !== (await sha256(oldPassword)))
